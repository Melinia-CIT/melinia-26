...
export const TabBar = ({ activeTab, onTabChange }: TabBarProps) => {
    ...
    return (
        <div className="relative group">
            <div
                className="flex gap-1 p-1 rounded-full border border-white/10 shadow-lg overflow-x-auto scrollbar-hide max-w-full relative backdrop-blur-xl hover:border-[#752B65]/30 transition-all duration-500"
                style={{
                    ...
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={`px-4 md:px-6 py-1.5 md:py-2 rounded-full text-xs md:text-sm font-semibold transition-all duration-300 whitespace-nowrap ${activeTab === tab.id
                                    ? 'bg-gradient-to-r from-[#D24623] to-[#EA8427] text-[#0F0B13] shadow-lg shadow-[#D24623]/40 scale-105 animate-bounce-subtle'
