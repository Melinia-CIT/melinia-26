...
const EventDetail = () => {
    ...
    return (
        <main className="flex-1 w-full p-4 md:px-8 md:py-6 relative">
            <div className="fixed top-0 left-0 right-0 z-[100] p-4 md:px-8 pointer-events-none">
                <div className="max-w-7xl mx-auto flex items-center h-12">
                    <motion.button onClick={() => navigate("/app/events")} className="pointer-events-auto flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-zinc-900/80 backdrop-blur-xl border border-white/10 text-zinc-400 hover:text-white transition-all shadow-2xl" whileHover={{ x: -3 }}>
            ...
            <motion.div className={`relative bg-gradient-to-br ${theme.banner} rounded-xl p-6 md:p-8 mb-6 overflow-hidden border border-white/10`} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                                        ...
                                        <div key={round.roundNo} className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
                                            ...
                                            >
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <h3 className="text-xs font-bold text-white uppercase tracking-tight">{round.roundName || `Round ${round.roundNo}`}</h3>
                                                            ...
                                                            {roundRules.length > 0 ? (
                                                                roundRules.map(rule => (
                                                                    <div key={rule.id} className="text-[11px] text-zinc-300 flex gap-2 bg-white/5 p-2 rounded border border-white/5">
                                                                        <span className={`${theme.accent} font-bold`}>{rule.ruleNumber}.</span>{rule.ruleDescription}
                                                                    </div>
                                                                ))
                                                            ) : (
                ...
                <div className="space-y-4">
                    <div className="bg-zinc-900/60 backdrop-blur-xl border border-white/10 rounded-2xl p-5 sticky top-20">
                        ...
                        >
                            {isRegistered ? <><CheckCircle2 className="w-5 h-5" /> Registered</> : status.text === "Open" ? "Register Now" : status.text}
                        </button>
                                ...
                                {event.organizers.map((org, i) => (
                                    <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-3">
                                        ...
                                        <div className="flex items-center gap-2 mt-1.5 pt-1.5 border-t border-white/5">
                                            ...
                                            <a href={`tel:${org.phoneNo}`} className="text-[10px] text-zinc-400 font-medium hover:text-white transition-colors">{org.phoneNo}</a>
                                        </div>
                                    </div>
                                ))}
                            </div>
