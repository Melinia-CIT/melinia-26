...
export function FloatingPathsBackground({
    ...
    const mirroredPaths = mirror
        ? Array.from({ length: 36 }, (_, i) => ({
              id: i + 36,
              d: `M${1076 + i * 5 * position} -${189 + i * 6}C${
                  1076 + i * 5 * position
              } -${189 + i * 6} ${1008 + i * 5 * position} ${216 - i * 6} ${
                  544 + i * 5 * position
              } ${343 - i * 6}C${80 + i * 5 * position} ${470 - i * 6} ${
                  12 + i * 5 * position
              } ${875 - i * 6} ${12 + i * 5 * position} ${875 - i * 6}`,
              color: `rgba(15,23,42,${0.1 + i * 0.03})`,
              width: 0.5 + i * 0.03,
          }))
        : []
                ...
                <svg className="w-full h-full" viewBox="0 0 696 316" fill="none" aria-hidden="true">
                    ...
                    {[...paths, ...mirroredPaths].map(path => (
                        <motion.path
                            ...
                            strokeOpacity={0.3 + (path.id % 36) * 0.015}
