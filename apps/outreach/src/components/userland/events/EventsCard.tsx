...
interface Organizer { userId: string; assignedBy: string; }
...
const EventsCard = ({ event }: EventsCardProps) => {
    ...
    const getThemeStyles = (type: string) => {
        ...
        switch (normalizedType) {
            case "technical":
                return {
                    header: "from-orange-400/20 via-rose-600/10 to-transparent",
            ...
            >
                <div className="absolute inset-0 z-50 pointer-events-none">
                    <svg className="w-full h-full" fill="none" preserveAspectRatio="none">
                        <defs>
                            <filter id={`video-glow-${event.id}`} x="-20%" y="-20%" width="140%" height="140%">
                                <feGaussianBlur stdDeviation="5" result="blur" />
                                <feFlood floodColor={theme.accent} floodOpacity="0.8" result="glowColor" />
                ...
                <div className={`relative h-28 overflow-hidden bg-gradient-to-br ${theme.header}`}>
                    ...
                    <div className="absolute top-4 right-4 flex flex-col items-end gap-2">
                        <motion.span className={`px-3 py-1 rounded-lg text-xs font-semibold border ${theme.badge}`}>
