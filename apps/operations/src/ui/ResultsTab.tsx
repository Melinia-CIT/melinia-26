...
export function ResultsTab({
    ...
    async function handleFlushWinners() {
        ...
        try {
            ...
            const msg = (err as AxiosError<{ message?: string }>).response?.data?.message ?? "Failed to flush winners"
            console.error(msg)
            setShowFlushConfirm(false)
        } finally {
            setIsFlushing(false)
    ...
    const { data: winnersData, isLoading: isWinnersLoading, error: winnersError, refetch: refetchWinners } = useQuery({
    ...
    const { data, isLoading, error, refetch } = useQuery({
        ...
        queryFn: () => api.events.getRoundResults(eventId, roundNo, { status: statusFilter === "all" ? undefined : statusFilter, sort, from: page * limit, limit }),
    ...
    const { data: fullData, isLoading: isFullLoading } = useQuery({
        ...
        queryFn: () => api.events.getRoundResults(eventId, roundNo, { status: statusFilter === "all" ? undefined : statusFilter, sort, from: 0, limit: 9999 }),
        enabled: !!activeSearch,
        staleTime: 1000 * 30,
    })
    ...
    const filteredResults = useMemo(() => {
        ...
        return fullData.data.filter((r: RoundResultWithParticipant) =>
            r.name.toLowerCase().includes(q) || r.user_id.toLowerCase().includes(q) ||
            r.ph_no.includes(q) || r.team_name?.toLowerCase().includes(q) || r.team_id?.toLowerCase().includes(q)
        )
    ...
    const paginatedResults = activeSearch ? filteredResults.slice(page * limit, (page + 1) * limit) : results
        ...
        <div>
            ...
            <div className="flex items-center gap-2 px-4 md:px-6 py-3 border-b border-neutral-800 bg-neutral-900/30">
                <div className="relative flex-1 max-w-md">
                    ...
                    <input type="text" placeholder="Search by name, ID, phone, or team..."
                    ...
                    {searchInput && (
                        <button type="button" onClick={onClearSearch} className="absolute right-3 top-1/2 -translate-y-1/2 text-neutral-500 hover:text-white">
                ...
                {activeSearch && <span className="text-xs text-neutral-500">Showing {totalCount} result{totalCount !== 1 ? "s" : ""} for "{activeSearch}"</span>}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 md:px-6 py-3 border-b border-neutral-800 bg-neutral-900/30">
                <div className="flex items-center gap-1.5 flex-wrap">
                    {(["all", "QUALIFIED", "ELIMINATED", "DISQUALIFIED"] as const).map(s => {
                        ...
                        return (
                            <button key={s} type="button" onClick={() => { setStatusFilter(s); onSetPage(0) }}
                                ...
                                {s !== "all" && statusCounts[s] != null && <span className="ml-1 opacity-70">({statusCounts[s]})</span>}
                                {s === "all" && totalCount > 0 && <span className="ml-1 opacity-70">({totalCount})</span>}
                            </button>
                        )
                    })}
                </div>
                <div className="ml-auto flex items-center gap-2">
                    ...
                    <select value={sort} onChange={e => { setSort(e.target.value as typeof sort); onSetPage(0) }}
            ...
            {/* Content */}
            {isLoadingResults && <div className="p-12 text-center text-neutral-500">Loading results…</div>}
            ...
            {!isLoadingResults && !error && paginatedResults.length === 0 && (
                <div className="p-12 text-center text-neutral-500">
                    {activeSearch ? `No results found for "${activeSearch}"` : statusFilter === "all" ? "No results recorded for this round yet." : `No ${statusFilter.toLowerCase()} participants.`}
                </div>
            )}
            {!isLoadingResults && !error && paginatedResults.length > 0 && (
                <>
                    ...
                    <div className="md:hidden divide-y divide-neutral-800 border border-neutral-800 bg-neutral-950 shadow-xl overflow-hidden">
                        {groupedResults.map(g => <ResultCardGroup key={g.id} members={g.members} />)}
                    </div>
                    {/* Desktop */}
                                ...
                                <thead>
                                    <tr className="border-b border-neutral-800 bg-neutral-900/60">
                                        {participationType?.toUpperCase() === "SOLO" ? (
                                            ...
                                            <>
                                                <th className="px-6 py-3 text-left text-[10px] font-semibold text-neutral-400 uppercase tracking-widest w-[200px] border-r border-neutral-800/60">Entity</th>
                                                <th className="px-6 py-3 text-left text-[10px] font-semibold text-neutral-400 uppercase tracking-widest">Participants</th>
                                            </>
                                        )}
                                        <th className="px-6 py-3 text-left text-[10px] font-semibold text-neutral-400 uppercase tracking-widest w-[240px] border-l border-neutral-800/60">Evaluation</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {groupedResults.map(g => <ResultRowGroup key={g.id} members={g.members} participationType={participationType} />)}
                                </tbody>
                            </table>
                        </div>
                    </div>
                    <TablePagination page={page} totalPages={totalPages} total={totalCount} pageLimit={limit}
                        onPrev={() => onSetPage(Math.max(0, page - 1))} onNext={() => onSetPage(page + 1)}
                        onSetPage={onSetPage} onSetLimit={onSetLimit} />
                </>
            )}

            <ConfirmDialog open={showFlushConfirm} title="Flush Winners?"
                ...
                confirmLabel="Flush All" isPending={isFlushing}
                onConfirm={handleFlushWinners} onCancel={() => setShowFlushConfirm(false)} />
        </div>
    )
}
