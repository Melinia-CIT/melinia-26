...
export function CheckedInTable({
    ...
    const {
        selected, allSelected, someSelected, entryMap,
        toggleAll, toggleEntry, clearSelection, handleToggleEntryWithPlaceContext,
        isAutoMode, toggleAutoMode,
        feedback, setFeedback, isPendingBatch, applyStatus,
        pendingDelete, setPendingDelete, deleteMutation,
        winners, pendingPlaceTarget, prizeFeedback, setPrizeFeedback,
        isSubmittingPrizes, handlePlaceClick, clearWinnerSlot, submitWinners,
    ...
    return (
        <div className="relative">
            ...
            {/* Feedback Banner */}
            {feedback && (
                <div className={`px-4 md:px-6 py-3 border-b text-xs font-medium flex flex-col gap-2 ${feedback.kind === "success" || feedback.kind === "delete-success"
                    ...
                    <div className="flex items-start justify-between gap-3">
                        ...
                        <button type="button" onClick={() => setFeedback(null)}
                            className="text-current opacity-60 hover:opacity-100 shrink-0 text-base leading-none">✕</button>
                    </div>
                    {(feedback.kind === "partial" || feedback.kind === "failure") && (
                        <div className="space-y-0.5 pl-2 border-l-2 border-current/30">
                            {feedback.userErrors.map(e => <div key={e.user_id} className="font-mono opacity-80">{e.user_id}: {e.error}</div>)}
                            {feedback.teamErrors.map(e => <div key={e.team_id} className="font-mono opacity-80">{e.team_id}: {e.error}</div>)}
                        </div>
                    )}
                </div>
            )}
            ...
            {!isLastRound && (
                <>
                    ...
                    <div className="flex items-center gap-3 px-4 md:px-6 py-2 border-b border-neutral-800/60 bg-neutral-950">
                        <span className="text-[10px] text-neutral-600 uppercase tracking-widest font-bold">Assignment Mode</span>
                        <div className="flex items-center border border-neutral-800 overflow-hidden">
                            {(["Auto", "Manual"] as const).map((label, i) => {
                                ...
                                return (
                                    <button key={label} type="button" onClick={() => toggleAutoMode(i === 0)}
                        ...
                        <span className="text-[10px] text-neutral-700 italic">
                            {isAutoMode ? "Qualifying auto-eliminates others, and vice versa" : "Only selected entries are updated"}
                        </span>
                    </div>

                    {/* Selection action bar */}
                    <div className={`sticky top-0 z-20 transition-all duration-300 ease-out overflow-hidden ${selectedCount > 0 ? "max-h-32 opacity-100" : "max-h-0 opacity-0 pointer-events-none"}`}>
                        <div className="bg-neutral-900 border-b border-neutral-700 px-4 md:px-6 py-3 flex items-center gap-3 flex-wrap shadow-lg shadow-black/40">
                            <div className="flex items-center gap-2 flex-1 min-w-0">
                                <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-white text-black text-[11px] font-black shrink-0">{selectedCount}</span>
                                <span className="text-xs text-neutral-300 font-medium">{selectedCount === 1 ? "entry" : "entries"} selected</span>
                            </div>
                            <div className="flex items-center gap-1.5 flex-wrap shrink-0">
                                {isPendingBatch && <span className="text-[11px] text-neutral-500 uppercase tracking-widest animate-pulse mr-1">Saving…</span>}
                                {(["QUALIFIED", "ELIMINATED", "DISQUALIFIED"] as const).map(s => {
                                    const cls = s === "QUALIFIED"
                                    ...
                                    const dot = s === "QUALIFIED" ? "bg-emerald-400" : s === "ELIMINATED" ? "bg-amber-400" : "bg-red-400"
                                    return (
                                        <button key={s} type="button" disabled={isPendingBatch} onClick={() => applyStatus(s)}
                                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest border disabled:opacity-40 disabled:cursor-not-allowed transition-colors duration-150 focus-visible:outline-none focus-visible:ring-1 ${cls}`}>
                                            <span className={`w-1.5 h-1.5 rounded-full ${dot} inline-block`} />
                                            {s.charAt(0) + s.slice(1).toLowerCase()}
                                            {s !== "DISQUALIFIED" && isAutoMode && <span className="opacity-50 text-[9px] normal-case tracking-normal font-normal ml-0.5">+auto</span>}
                                        </button>
                                    )
                                })}
                                <button type="button" disabled={isPendingBatch} onClick={clearSelection}
            ...
            {/* States */}
            {isLoading && <div className="p-12 text-center text-neutral-500">Loading checked-in participants...</div>}
            {error && !isLoading && (
            ...
            {!isLoading && !error && data.length === 0 && (
                <div className="p-12 text-center text-neutral-500">
                    {activeSearch ? `No results found for "${activeSearch}"` : "No participants have checked in yet."}
                </div>
            )}
            ...
            {!isLoading && !error && data.length > 0 && (
                <div className="md:hidden divide-y divide-neutral-800">
                    {data.map((entry, idx) => {
                        ...
                        const assignedPlace = (() => { for (const [p, s] of winners.entries()) { if (s.entryId === id) return p as PrizeSlot } return null })()
                        return (
                            <CheckedInCard key={idx} entry={entry} checked={selected.has(id)}
                                onToggle={() => isLastRound ? handleToggleEntryWithPlaceContext(id) : toggleEntry(id)}
                                assignedPlace={assignedPlace} isLastRound={isLastRound}
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-neutral-800 bg-neutral-900/60 font-mono">
                                    ...
                                    {participationType?.toUpperCase() === "SOLO" ? (
                                        <th colSpan={2} className="px-6 py-3 text-left text-[10px] font-semibold text-neutral-400 uppercase tracking-widest border-r border-neutral-800/60">Participant Details</th>
                                    ) : (
                                        <>
                                            <th className="px-6 py-3 text-left text-[10px] font-semibold text-neutral-400 uppercase tracking-widest border-r border-neutral-800/60 w-[200px]">Team / Entry</th>
                                            <th className="px-6 py-3 text-left text-[10px] font-semibold text-neutral-400 uppercase tracking-widest">Participant Name</th>
                                        </>
                                    )}
                                    <th className="px-6 py-3 text-left text-[10px] font-semibold text-neutral-400 uppercase tracking-widest">College &amp; Degree</th>
                                    <th className="px-6 py-3 text-left text-[10px] font-semibold text-neutral-400 uppercase tracking-widest">Phone Number</th>
                            ...
                            <tbody>
                                {data.map((entry, idx) => {
                                    ...
                                    const assignedPlace = (() => { for (const [p, s] of winners.entries()) { if (s.entryId === id) return p as PrizeSlot } return null })()
                                    return (
                                        <CheckedInRowGroup key={idx} entry={entry} checked={selected.has(id)}
                                            onToggle={() => isLastRound ? handleToggleEntryWithPlaceContext(id) : toggleEntry(id)}
                                            assignedPlace={assignedPlace} isLastRound={isLastRound}
                                            onDelete={() => setPendingDelete(entry)} />
                                    )
            ...
            <TablePagination page={page} totalPages={totalPages} total={total} pageLimit={pageLimit}
                onPrev={onPrev} onNext={onNext} onSetPage={onSetPage} onSetLimit={onSetLimit} />
